<script lang="ts">
	...
	const { data }: { data: { available: boolean } } = $props();
	let email = $state('');
...
<div>
	...
	{#if !data.available}
		<p class="mt-4 text-sm leading-6 text-muted" role="status">Password reset is temporarily unavailable. Please contact support for help accessing your account.</p>
	{:else if submitted}
