...
export type OutboxKind = 'message.send' | 'webhook.event';
...
export type OutboxHandlers = {
	'message.send': (sql: Sql, provider: MessagingProvider, payload: Record<string, unknown>) => Promise<void>;
	'webhook.event': (sql: Sql, provider: MessagingProvider, payload: Record<string, unknown>) => Promise<void>;
};
...
export async function drainOutbox(
	...
	while (processed < limit) {
		...
		try {
			...
			await handler(sql, provider, job.payload);
