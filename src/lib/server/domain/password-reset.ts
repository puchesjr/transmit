...
export async function requestPasswordReset(
	...
	return sql.begin(async (sql) => {
		await sql`select pg_advisory_xact_lock(hashtextextended(${`password-reset:${input.email.toLowerCase()}`}, 0))`;
...
export async function confirmPasswordReset(
	...
	const passwordHash = await hashPassword(parsePassword(input.password));
	return sql.begin(async (tx) => {
		const reset = await findUsablePasswordReset(tx, hashToken(input.token));
